...
static uint8_t crc5(uint8_t *data, uint16_t len_bytes)
{
    ...
    uint8_t din = 0;
    uint16_t len_bits = len_bytes * 8;

    for (j = 0x80, k = 0, i = 0; i < len_bits; i++) {
        din = (data[index] & j) != 0;
                              ...
                              const uint8_t *data, uint8_t data_len)
{
    ...
    size_t total = job ? (size_t)data_len + 6 : (size_t)data_len + 5;
    uint8_t *buf;

    if (total > 261) {
        applog(LOG_ERR, "BM1370: packet too large: %zu bytes", total);
        return -1;
    }
    ...
    if (job) {
        ...
        buf[4 + data_len] = crc5(buf + 2, (uint16_t)data_len + 2);
    }

    if (write(info->uart.fd, buf, total) != (ssize_t)total) {
