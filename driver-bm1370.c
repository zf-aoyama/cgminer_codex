...
{
    static const uint8_t version_mask_cmd[6] = {0x00, 0xA4, 0x90, 0x00, 0x00, 0x00};
    static const uint8_t inactive_cmd[2]    = {0x00, 0x00};

    if (bm1370_send_packet(info, TYPE_CMD | GROUP_ALL | CMD_WRITE,
                           version_mask_cmd, sizeof(version_mask_cmd)))
        return false;

    if (bm1370_send_packet(info, TYPE_CMD | GROUP_ALL | CMD_INACTIVE,
                           inactive_cmd, sizeof(inactive_cmd)))
        return false;
